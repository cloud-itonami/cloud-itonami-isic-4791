(ns mailorderops.governor-contract-test
...
(deftest order-record-logging-full-flow
  (testing "clean order-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-1" :phase 3}
          result (exec-request actor "t1"
                               {:op :log-order-record :seller-id "seller-1" :patch {:orders-placed 42}}
                               ctx)]
      (is (some? result))
...
(deftest fraud-concern-always-escalates
  (testing ":flag-fraud-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-2" :phase 3}
          result (exec-request actor "t2"
                               {:op :flag-fraud-concern :seller-id "seller-1"
                                :patch {:concern "unusual order velocity" :confidence 0.99}}
                               ctx)]
      (is (some? result))
      ;; At this point the actor is paused for approval, not yet committed
      (is (= 0 (count (store/coordination-log db)))
          "fraud concern must not auto-commit, must wait for approval")
      ;; Now approve it
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :seller-id "seller-1"
                                :patch {:item "seasonal inventory build" :quantity 1 :estimated-cost 3200.0
                                        :vendor-id "vendor-1"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
  ...
  (testing "unregistered seller -> permanent HARD hold, never escalates"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t3"
                     {:op :log-order-record :seller-id "unknown-seller"
...
(deftest seller-not-payment-processor-linked-hard-hold
  (testing "registered+verified but NOT payment-processor-linked seller -> permanent HARD hold (e-commerce-specific flagship check)"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-4c" :phase 3}
          result (exec-request actor "t4c"
                               {:op :log-order-record :seller-id "seller-4"
                                :patch {:orders-placed 10}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
          "seller not yet linked to a payment processor must HARD hold"))))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into fraud/chargeback-finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8b"
                     {:op :log-order-record :seller-id "unknown" :patch {:orders-placed 42}}
                     ctx)
