(ns mailorderops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (MailOrderRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the MailOrderRetailGovernor (:govern)
  ...
  human seller/merchant operations coordinator. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mailorderops.advisor :as advisor]
            [mailorderops.governor :as governor]
...
(defn build
  ...
  `mailorderops.store/Store`).
  opts:
    :advisor      -- a `mailorderops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MailOrderRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; MailOrderRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :seller-id (:seller-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :seller-id (:seller-id request) :by (:by approval)}]}
            {:disposition :hold
